...
class CanvasView: UIView {
    ...
    func 问题1() {
        ...
        pencil.move(to: CGPoint(x: 12, y: 400 + h))
        pencil.addLine(to: CGPoint(x: 716, y: 400 + h))
        pencil.lineWidth = 5
        pencil.stroke()
        ...
        for i in 0..<120 {
            ...
            let y: CGFloat = 400 - a
            UIBezierPath(arcCenter: CGPoint(x: 364 - b , y: y) , radius: 3, startAngle: 0 * CGFloat.pi, endAngle: 2 * CGFloat.pi, clockwise: false).fill()
            UIBezierPath(arcCenter: CGPoint(x: 364 + b , y: y) , radius: 3, startAngle: 0 * CGFloat.pi, endAngle: 2 * CGFloat.pi, clockwise: false).fill()
        }
    }
